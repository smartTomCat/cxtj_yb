...
public class SetSearchController extends CxtjBaseController {
    ...
    public String toEditSearch() throws Exception {
        ...
        if (!ValidateUtil.isEmpty(zb61Domain)) {
            //赋值数据集下拉选
            List rsList = setSearchService.queryResultSetsByYzb670(zb61Domain.getYzb670().toString());
            setSelectInputList("yzb690", rsList);
            if (ValidateUtil.isNotEmpty(zb61Domain.getYzb690())) {
                //使用数据集
                setData("userrsesultset", "1");
                setReadOnly("yzb615,yzb616");
            }
        ...
        if (ValidateUtil.isNotEmpty(columnList)) {
            for (int i = 0; i < columnList.size(); i++) {
                for (int j = 0; j < columnList.size(); j++) {
                    if (i != j && columnList.get(i).getAsString("yzb623").equals(columnList.get(j).getAsString("yzb623"))) {
                        setMessage("存在重复字段：" + columnList.get(j).getAsString("yzb623"), "warn");
                        return JSON;
                    }
                }
            }
    ...
    public String getEditSearchItemSelect() throws Exception {
        ...
        if (ValidateUtil.isNotEmpty(yzb690)) {
            list = setSearchItemService.querySearchItemInResultSet(getTaDto());
        } else {
            list = setSearchItemService.querySearchItemSelect(getTaDto());
        }
        if (ValidateUtil.isEmpty(list)) {
            setMessage("表或视图不存在", "warn");
        }
        //去重
        List<Map> list2 = setSearchItemService.queryExistCols(getTaDto());
        Iterator iterator = list.iterator();
        while (iterator.hasNext()) {
            Map m =(Map)iterator.next();
            for (Map m2 : list2) {
                if (m.get("yzb623").equals(m2.get("yzb623"))) {
                    iterator.remove();
                }
            }
        }

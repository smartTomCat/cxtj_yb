...
import com.yinhai.cxtj.front.service.PgCustomizeQueryService;
 ...
 */
public class PgCustomizeQueryServiceImpl extends CommonCustomizeQueryServiceImpl implements PgCustomizeQueryService {
                    ...
                    for(int i=0;i<funcitonFieldList.size();i++){
                        ...
                        if("1".equals(tjfs)){//计数
                            ...
                            if("1".equals(dbtype)){      //字符型
                                sql.append("COALESCE(").append(filedName).append(",'0'").append(")");
                            }else if("2".equals(dbtype)){//数字型
                                sql.append("COALESCE(").append(filedName).append(",0").append(")");
                            }else if("3".equals(dbtype)){//日期型
                                sql.append("COALESCE(").append(filedName).append(",CURRENT_DATE").append(")");
                            }else{
                            ...
                            if("2".equals(dbtype)){//数字型
                                sql.append("AVG(COALESCE(").append(field.get("id")).append(",0))");
                            }else {
                                    ...
                                    for(int j=0;j< andList.size();j++){
                                        ...
                                        if(ValidateUtil.isNotEmpty(datatype)){
                                            ...
                                            if("1".equals(vtype)){//值类型
                                                if("11".equals(datatype)){//文本
                                                    if("11".equals(andMap.get("gxysf"))){//等于
                                                        ...
                                                        if(p.matcher(_value).find()){
                                                            for(int k = 0; k < _valueA.length; k++){
                                                                if(ValidateUtil.isNotEmpty(_valueA[k])){
                                                                    sql.append(" OR").append(" POSITION(").append(andMap.get("id")).append(" IN '").append(_valueA[k]).append("') > 0");
                                                                }
                                                            }
                                                        }
                                                        sql.append(")");
                                                        ...
                                                        if(p.matcher(_value).find()){
                                                            for(int k = 0; k < _valueA.length; k++){
                                                                if(ValidateUtil.isNotEmpty(_valueA[k])){
                                                                    sql.append(" AND").append(" POSITION(").append(andMap.get("id")).append(" IN '").append(_valueA[k]).append("') = 0");
                                                                }
                                                            }
                                                        }
                                                        sql.append(")");
                                                ...
                                                sql.append(" ''|| ").append(andMap.get("id")).append("");
                                                if("11".equals(andMap.get("gxysf"))){//等于
                                                    sql.append(" = ");
                                                    sql.append(" ''|| ").append(andMap.get("vfield")).append("");
                                                }else if("12".equals(andMap.get("gxysf"))){//不等于
                                                    sql.append(" != ");
                                                    sql.append(" ''|| ").append(andMap.get("vfield")).append("");
                                                }else if("21".equals(andMap.get("gxysf"))){//大于
                                                    ...
                                                    sql.append(" IN( ''|| ").append(andMap.get("vfield")).append(")");
                                                }else if("42".equals(andMap.get("gxysf"))){//不包含
                                                    sql.append(" NOT IN( ''|| ").append(andMap.get("vfield")).append(")");
                                                }
                                            }
                                        }
                                    }

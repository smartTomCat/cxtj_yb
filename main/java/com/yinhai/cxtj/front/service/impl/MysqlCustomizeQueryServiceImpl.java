...
import com.yinhai.cxtj.front.service.MysqlCustomizeQueryService;
 ...
 */
public class MysqlCustomizeQueryServiceImpl extends CommonCustomizeQueryServiceImpl implements MysqlCustomizeQueryService {
    ...
    public Map getPageStatisticalData(TaParamDto para) throws Exception {
        ...
        if(ValidateUtil.isNotEmpty((String)outmap.get("sql"))){
            ...
            try{
                ...
                yzb996 += "^|^" + String.valueOf(map.get("sql"));
                if (ValidateUtil.isNotEmpty(list)) {
                    outputDomain.put("list",list);
                }
                Map fieldData =  new HashMap();
                if(null!=outMap1 && null!=outMap1.get("count")){
                    fieldData.put("datacount",outMap1.get("count"));
                }
                fieldData.put("fileds",outmap.get("fileds"));
                fieldData.put("tjfalsh", outmap.get("tjfalsh"));
                //返回页面构件datagrid
                outputDomain.put("fieldData",fieldData);
            }catch (Exception e) {
                yzb994 = "1";
                yzb995 = e.getMessage();
                e.printStackTrace();
                //异常在controller 通过appcode判断获取
            }finally {
                if("1".equals(yzb994)){
                    outputDomain.put("appCode", "-1");
                    outputDomain.put("appMsg", "sql查询执行失败!!! " + yzb995);
                }
                    ...
                    for(int i=0;i<funcitonFieldList.size();i++){
                        ...
                        if("1".equals(tjfs)){//计数
                            ...
                            if("1".equals(dbtype)){      //字符型
                                sql.append("IFNULL(").append(filedName).append(",'0'").append(")");
                            }else if("2".equals(dbtype)){//数字型
                                sql.append("IFNULL(").append(filedName).append(",0").append(")");
                            }else if("3".equals(dbtype)){//日期型
                                sql.append("IFNULL(").append(filedName).append(",SYSDATE()").append(")");
                            }else{
                            ...
                            if("2".equals(dbtype)){//数字型
                                sql.append("AVG(IFNULL(").append(field.get("id")).append(",0))");
                            }else {
                                        ...
                                        if(ValidateUtil.isNotEmpty(datatype)){
                                            ...
                                            if("1".equals(vtype)){//值类型
                                                if("11".equals(datatype)){//文本
                                                    ...
                                                    sql.append("date_format(").append(andMap.get("id")).append(",'%Y-%m')");
                                                    if("11".equals(andMap.get("gxysf"))){//等于
                                                        sql.append(" = ");
                                                    }else if("12".equals(andMap.get("gxysf"))){//不等于
                                                        sql.append(" != ");
                                                    ...
                                                    sql.append("date_format(").append(andMap.get("id")).append(",'%Y-%m-%d')");
                                                    if("11".equals(andMap.get("gxysf"))){//等于
                                                        sql.append(" = ");
                                                    }else if("12".equals(andMap.get("gxysf"))){//不等于
                                                        sql.append(" != ");
                                                ...
                                                sql.append("CONCAT(").append(andMap.get("id")).append(",'')");
                                                if("11".equals(andMap.get("gxysf"))){//等于
                                                    sql.append(" = ");
                                                    sql.append("CONCAT(").append(andMap.get("vfield")).append(",'')");
                                                }else if("12".equals(andMap.get("gxysf"))){//不等于
                                                    sql.append(" != ");
                                                    sql.append("CONCAT(").append(andMap.get("vfield")).append(",'')");
                                                }else if("21".equals(andMap.get("gxysf"))){//大于
                                                    ...
                                                    sql.append(" IN(CONCAT(").append(andMap.get("vfield")).append(",''))");
                                                }else if("42".equals(andMap.get("gxysf"))){//不包含
                                                    sql.append(" NOT IN(CONCAT(").append(andMap.get("vfield")).append(",''))");
                                                }
                                            }
                                        }
                                    }

...
<head>
    ...
    <script src="<%=facePath%>lib/echart/echarts-all.js" type="text/javascript"></script>
...
<body>
<ta:form id="queryId" methord="post">
        ...
        <div id="gruops_id">
            <table id="datagrid_query_id" width="100%" align="center" cellpadding="2" cellspacing="0" border="0">
                ...
                <tr>
                    ...
                    <td style="width:50px;">
                            <%-- <s:select id="" name="cxxm" list="#request.ztxmList"  listKey="yzb620" listValue="yzb625" cssStyle="width:97%;" onchange="getItemGxOrNr(this)"/>
                             --%>

                        <select name="cxxm" style="width:97%;" onchange="getItemGxOrNr(this)">
                            <option value=''>---请选择---</option>
                            <s:iterator value="#request.ztxmList" id="item" status="st">
    ...
    <ta:box cols="5" cssStyle="">
            ...
            <div id="tafieldset_49793242" layout="column" >
                ...
                <div id="groupItems_id_box" class="tafieldset_163">
                    <div id="groupByItemsId">
                        <s:iterator value="#request.enableTjItems" id="item" status="st">
                            <div>
                                <input type="checkbox" name="tjxm" datatype="<s:property value="#item.yzb62a"/>"
                                       fname="<s:property value="#item.yzb625" />"
                                       ztxmlsh="<s:property value="#item.yzb620" />"
                                       value="<s:property value="#item.yzb623" />"
                                        <s:if test="#item.yzb62b==1">
                                            checked="checked"
                                        </s:if>/><s:property value="#item.yzb625"/>
                            </div>
                        </s:iterator>
                    </div>
        ...
        <div class="ez-fl ez-negmx" style="width:43.0%;">
            <div id="tafieldset_49793243" layout="column" span="2.7">
                <div class="tafieldset_header ">
                    <div class="header-bg"></div>
                ...
                <div style="height:158px;width:100%;overflow:auto;">
                    <table id="datagrid_query_id" width="100%" align="center" cellpadding="2" cellspacing="0" border="0">
                        ...
                        <s:if test="#request.mrfzjsxmList != null && #request.mrfzjsxmList.size > 0"><!-- 有配置默认统计列时 -->
                        <s:iterator value="#request.mrfzjsxmList" id="mrItem" status="msSt">
                            <tr>
                                <td style="width:50px;">
                                ...
                                <td style="width:250px;">
                                    <select name="tjzb" style="width:97%;" onchange="getXmzcys(this)">
                                        <option value=''>---请选择---</option>
                                        <s:iterator value="#request.tjzbhsList" id="item" status="st">
                                            <option value='<s:property value="#item.yzb620"/>'
                                                    filed='<s:property value="#item.yzb623"/>'
                                                    dbtype='<s:property value="#item.yzb626"/>'
                                                    datatype='<s:property value="#item.yzb62a"/>'
                                                    <s:if test="#item.yzb620 == #mrItem['yzb620']">selected="selected"</s:if>>
                                                <s:property value="#item.yzb625"/>
                                            </option>
                                        </s:iterator>
                                    </select>
                                ...
                                <td style="width: 45%">
                                    <select name="tjzbhs" style="width:97%;">
                                        <s:iterator value="#mrItem['collection']" id="item" status="st">
                                            <option value='<s:property value="#item.yzb641"/>'
                                                    <s:if test="#item.yzb641 == #mrItem['yzb641']"> selected="selected"</s:if>>
                        ...
                        <s:elseif test="#request.mrfzjsxmList == null || #request.mrfzjsxmList.size == 0"><!-- 没有配置默认统计列时 -->
                        <tr>
                            <td style="width:50px;">
                                <a href="javascript:void(0);" onclick="deleteTjzbhsTr(this)">删除</a>
                            </td>
                        ...
                        <td style="width:30px;text-align: left;" colspan="3">
                            <ta:button key="增加统计指标" onClick="addTjzbhsTr(this)" cssClass="btnadd"/>
                        </td>
                        </tbody>
                    </table>
                </div>
    ...
    <ta:box cols="6" cssStyle="">
        <ta:box span="2" cssStyle="height: 38px"/>
        <ta:box span="1.5">
            <ta:radiogroup id="showTypes" span="1.5" cols="5">
                <ta:radio value="1" onClick="" key="表格&nbsp;&nbsp;" checked="true"></ta:radio>
                <ta:radio value="2" onClick="" key="柱状图" cssStyle="margin-right:2px;"></ta:radio>
                <ta:radio value="3" onClick="" key="饼图"></ta:radio>
                <ta:radio value="4" onClick="" key="折线图"></ta:radio>
            </ta:radiogroup>
        </ta:box>
        <ta:box span="2" cssStyle="margin-left:-20px;position: relative;">
            <ta:button key="&nbsp;&nbsp;&nbsp;&nbsp;统&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;计&nbsp;&nbsp;&nbsp;&nbsp;"
...
<div id="detailInfoFiledTitleId"></div>

<!-- 点击详细信息生成列头排序html -->
<div id="detailInfoFiledOrderId">
    <s:iterator value="#request.orderXms" id="item" status="st">
        <input type="hidden" name="detailOrderFileds"
               sn='<s:property value="#item.yzb651"/>'
...
//                }));
    ...
    function fnQueryFileds() {
        ...
        $.each($("select[name='tjzb']"), function (sn, _o) {//统计指标
            _o = $(_o);
            var ztxmlsh = $(_o).val() || '';
            if (ztxmlsh != '' && $.inArray(ztxmlsh, ztxmlshs) == -1) {//项目编号不能为空、未生成过的项目

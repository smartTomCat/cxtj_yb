...
<%
	...
	boolean isShowYab003ByTreeLevelView =SysConfig.getSysConfigToBoolean("isShowYab003ByTreeLevelView",false);
	boolean isHirarchyrgOrgType = SysConfig.getSysConfigToBoolean("isHirarchyrgOrgType",false);
%>
<html>
<head>
	<title>新增组织</title>
...
<ta:form id="orgForm" fit="true">
	<ta:panel id="orgInfo" hasBorder="false" expanded="false" scalable="false" fit="true"
		...
		<ta:box id="departPanel" cssStyle="padding-top:10px;padding-right:20px;" fit="true">
			...
			<ta:text id="porgname" key="父组织名称" readOnly="true" columnWidth="0.6" span="2" labelWidth="115" />
			<ta:text id="orgnamepath" key="父组织路径" readOnly="true" columnWidth="0.6" span="2" disabled="true" labelWidth="115"/>
			<ta:text id="orgname" key="组织名称" validType="[{type:'maxLength',param:[60],msg:'最大长度为60'}]" required="true" columnWidth="0.6" span="2"  labelWidth="115"/>
			<ta:box span="2" id="yab003TreeView"  cssStyle="padding-left:12px">
				<ta:selectTree  url="orgUserMgAction!getYab003List.do" required="true" asyncParam="['id']"
								...
								treeId="w_yab003Tree" targetId="yab003Tree"  targetDESC="w_yab003Desc" />
			</ta:box>
			<ta:box span="2" columnWidth="0.6" >
				<ta:text id="costomno" key="自定义编码" columnWidth="1" validType='[{type:"maxLength",param:[20],msg:"最大长度为20"},{type:"compare",param:[">","maxUdi"]}]' labelWidth="115"  />
			...
			<%--<ta:selectData labelWidth="115" placeholder="输入姓名后查询" hiddenValue="positionid" id="positionnamelike"  name="orgmanager" submitIds="orgid" displayValue="positionname" key="组织负责人(正职)" columnWidth="0.6" span="2" url="orgUserMgAction!queryLikeZhengzhi.do" inputQueryNum="1" titleValue="orgnamepath"></ta:selectData>
			<ta:selectData labelWidth="115" placeholder="输入姓名后查询" hiddenValue="positionid" id="positionnamelike1" name="orgmanager_deputy" submitIds="orgid" displayValue="positionname" key="组织负责人(副职)" columnWidth="0.6" span="2" url="orgUserMgAction!queryLikeZhengzhi.do" multiple="true" inputQueryNum="1" titleValue="orgnamepath"/>
--%>
			<ta:selectInput id="yab003" collection="yab003" key="经办机构" required="true" span="2" labelWidth="115" columnWidth="0.6" filterOrg="false"  />
			<ta:radiogroup id="effective" labelWidth="115" key="禁用标志"  cols="2" required="true" span="2" columnWidth="0.6">
				<ta:radio name="dto['effective']" key="禁用" value="0" />
				<ta:radio name="dto['effective']" key="启用" value="1" />
			</ta:radiogroup>
			<%-- 新增组织扩展jsp --%>
			<%@include file="../../org/orgextend/orgMgExtend.jsp" %>
		</ta:box>
		<ta:panelButtonBar>
			<ta:button type="submit"  url="orgUserMgAction!addOrg.do" id="save" key="保存[S]" onSubmit="fnBeforeSaveOrg" submitIds="orgForm" hotKey="S" icon="icon-add1" successCallBack="fnSaveSuccessCb" />
...
<script  type="text/javascript">
	...
	function fnOrgTypeSelect(value,key){
		...
		var isHierarchyOrgType ='<%=isHirarchyrgOrgType%>';
		if('true' != isHierarchyOrgType)
			return;
